...
/**
 * Design tokens shared with the RideX rider app, byte for byte apart from the duty block below.
 * Two apps, one product: a partner screen must look like it came from the same studio.
 *
 * Read off the RideX mockups. Every colour, radius and type size in the app
...
export const colors = {
  ...
  // Partner-only. Duty status and document review have no rider equivalent.
  /** On duty. The mint already reads as "live", so duty state reuses it rather than inventing one. */
  online: '#2EE7C7',
  /** Off duty. Deliberately dull - off duty should look off, not like a second brand colour. */
  offline: '#5A6478',
  /** Under review, expiring document, acceptance rate slipping. */
  warning: '#D9A05B',
  /** Trip completed, payout settled. Distinct from the mint so "earned" never reads as "live". */
  success: '#3DDC97',
  successSurface: '#0E2E24',
  dangerSurface: '#2C1520',
} as const;
