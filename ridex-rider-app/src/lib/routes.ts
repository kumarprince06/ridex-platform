...
/** Road geometry through every point in order - a shuttle's route through all its stops. */
export async function fetchRouteThrough(points: LngLat[], signal?: AbortSignal): Promise<LngLat[] | null> {
  if (points.length < 2) {
    return null;
  }
